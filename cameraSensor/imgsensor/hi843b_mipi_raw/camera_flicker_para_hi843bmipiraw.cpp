...
#include "camera_AE_PLineTable_hi843bmipiraw.h"
#include "camera_info_hi843bmipiraw.h"
...
{
	ALOGD("impGetFlickerPara+ mode=%d", sensorMode);//shannon
	ALOGD("prv=%d, vdo=%d, cap=%d, zsd=%d",
	    (int)e_sensorModePreview, (int)e_sensorModeVideoPreview, (int)e_sensorModeCapture, (int)e_sensorModeZsd );//shannon
	...
	{
		ALOGD("impGetFlickerPara ERROR ln=%d", __LINE__);//shannon
		return -1;
	}
	ALOGD("impGetFlickerPara-");//shannon
	return 0;
}
}
