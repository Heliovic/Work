...
public class AudioService extends IAudioService.Stub {
    ...
    // default: 1
    private int mNotificationUseRingVolume = 1;
    // -1 : not supported:always sync ring volume with notification volume
    //  0 : off: separate ring and notification volume
    //  1 : on : sync ring volume with notification volume
    private final String KEY_NOTIFICATION_USE_RING_VOLUME = "asus_notification_use_ring_volume";
    ...
    public AudioService(Context context) {
        ...
        // +++ Add by Morphy_Huang@2016/10/18 +++
        // [Asus Feature] Separate ring and notification volume
        readNotificationUseRingVolumeSetting();
        updateAsusStreamVolumeAlias();
        // --- Add by Morphy_Huang@2016/10/18 ---		
    ...
    // [Asus Feature]
    // If KEY_NOTIFICATION_USE_RING_VOLUME does not exist, follow the rules below:
    // Voice Capable Device : default 0
    // Other : -1 // always sync ring volume with notification volume
    private void readNotificationUseRingVolumeSetting() {
        Log.d(TAG, "NotificationUseRingVolume:"+mNotificationUseRingVolume);
        try {
               mNotificationUseRingVolume = Settings.System.getInt(mContext.getContentResolver(), KEY_NOTIFICATION_USE_RING_VOLUME);
        }
        catch (Settings.SettingNotFoundException e) {
               //modify mohongwu@wind-mobi.com 2016/12/23 start
               //mNotificationUseRingVolume =  System.getInt(mContext.getContentResolver(), KEY_NOTIFICATION_USE_RING_VOLUME,
               //                       mPlatformType == AudioSystem.PLATFORM_VOICE ? 1 : -1);
               mNotificationUseRingVolume =  System.getInt(mContext.getContentResolver(), KEY_NOTIFICATION_USE_RING_VOLUME,
                    mPlatformType == AudioSystem.PLATFORM_VOICE ? 0 : -1);
               //modify mohongwu@wind-mobi.com 2016/12/23 end
               Settings.System.putInt(mContext.getContentResolver(), KEY_NOTIFICATION_USE_RING_VOLUME, mNotificationUseRingVolume);
        }
    }

    private void updateAsusStreamVolumeAlias() {
       if (mPlatformType != AudioSystem.PLATFORM_VOICE) return;
    ...
    private void onSetStreamVolume(int streamType, int index, int flags, int device,
        ...
        // +++ Add by Morphy@2016/11/09 +++
        // Output SetStreamVolume log
        Log.d(TAG, "onSetStreamVolume:"+caller +" StreamType:"+ streamType+" index:"+index);
        // --- Add by Morphy@2016/11/09 ---
        ...
        try {
            synchronized (mSettingsLock) {
                ...
                if (external) {
                    ...
                    if (ringerMode != ringerModeInternal) {
                        setRingerModeInt(ringerMode, true /*persist*/);
                        Settings.Global.putInt(mContentResolver, Settings.Global.MODE_RINGER, ringerMode); // chilung_chen@asus.com, write to DB to prevent inconsistent error.
                    }
                    if (mRingerModeDelegate != null) {
    ...
    // +++ Add by Morphy_Huang@2016/11/17 +++
    // Sync ring volume to notification volume once setting changes 
    private void syncNotificationVolume() {
        int ringerMode = getRingerModeInternal();
        int ringVolume = getStreamVolume(mStreamVolumeAlias[AudioSystem.STREAM_RING]);
        if (ringerMode == AudioManager.RINGER_MODE_NORMAL) {
            setStreamVolume(AudioSystem.STREAM_NOTIFICATION, ringVolume, 0, mContext.getOpPackageName());
        } else {
            setStreamVolume(AudioSystem.STREAM_NOTIFICATION, getStreamMinVolume(AudioSystem.STREAM_NOTIFICATION), 0, mContext.getOpPackageName());
        }
    }

    // Update StreamState of STREAM_NOTIFICATION once setting changes
    private void updateStreamStates() {
        mStreamStates[AudioSystem.STREAM_NOTIFICATION] =
                  new VolumeStreamState(System.VOLUME_SETTINGS[mStreamVolumeAlias[AudioSystem.STREAM_NOTIFICATION]], AudioSystem.STREAM_NOTIFICATION);
    }
    // --- Add by Morphy_Huang@2016/11/17 ---
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        private final Uri NOTIFICATION_USE_RING_VOLUME_URI =
                        Settings.System.getUriFor(KEY_NOTIFICATION_USE_RING_VOLUME);
        ...
        SettingsObserver() {
            ...
            mContentResolver.registerContentObserver(NOTIFICATION_USE_RING_VOLUME_URI, false, this);
        }
        ...
        public void onChange(boolean selfChange,Uri uri) {
            ...
            if (NOTIFICATION_USE_RING_VOLUME_URI.equals(uri)) {
                if (mPlatformType == AudioSystem.PLATFORM_VOICE) {
                    readNotificationUseRingVolumeSetting();
                    if (mNotificationUseRingVolume == 0) {
                        updateAsusStreamVolumeAlias();
                        syncNotificationVolume();
                    } else {
                        syncNotificationVolume();
                        updateAsusStreamVolumeAlias();
                    }   
