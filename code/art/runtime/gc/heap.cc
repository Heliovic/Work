    ...
    case kCollectorTypeCMS: {
      if (IsMovingGc(collector_type_)) {
        ...
        CreateMainMallocSpace(mem_map.get(), kDefaultInitialSize,
                              std::min(capacity_, growth_limit_), capacity_);
#else
        CreateMainMallocSpace(mem_map.get(), kDefaultInitialSize,
        ...
        main_space_backup_.reset(CreateMallocSpaceFromMemMap(
            mem_map.get(), kDefaultInitialSize, std::min(capacity_, growth_limit_),
            capacity_, name, true));
#else
        main_space_backup_.reset(CreateMallocSpaceFromMemMap(
...
#if defined (HAVE_ANDROID_OS) && defined(MTK_FIX_GMO_BUG_CLAMPGROWLIMIT)
  // MTK: For GMO projects, race condition occurs when one thread does Heap::ClampGrowthLimit()
  // and another thread does Heap::TransitionCollector().
  // This fix can avoid this race issue to cause main_space_backup_.mem_map_ =0x0.
#else
  for (const auto& space : continuous_spaces_) {
    if (space->IsMallocSpace()) {
      gc::space::MallocSpace* malloc_space = space->AsMallocSpace();
...
// COE_SAFE_FUNCTION:Optional target size for Object.Clone()
