...
// COE_SAFE_FUNCTION:Disable GVN and LVN optimizations during recompilation
...
// COE_SAFE_FUNCTION:Disable method inlining
...
__attribute__((weak))
uint32_t InitCompilerOptimizerDisableFlags(uint32_t flag ATTRIBUTE_UNUSED) {
...
__attribute__((weak))
void TurnOffAdvancedOpt(CompilationUnit* cu ATTRIBUTE_UNUSED) {
}

__attribute__((weak))
bool MTKSkipCompilation(CompilationUnit& cu, std::string* skip_message ATTRIBUTE_UNUSED,
...
bool MTKCanCompileMethod(uint32_t method_idx, const DexFile& dex_file,
                         CompilationUnit* cu) {
  QuickCompiler* compiler = static_cast<QuickCompiler*>(cu->compiler_driver->GetCompiler());
  return compiler->CanCompileMethod(method_idx, dex_file, cu);
}
#endif
