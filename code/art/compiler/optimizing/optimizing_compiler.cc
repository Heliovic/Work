...
#ifdef MTK_ART_COMMON
  BoundsCheckElimination bce(graph, driver);
#else
...
#ifdef MTK_ART_COMMON
__attribute__((weak))
bool ShouldFallback(CompilerDriver* compiler_driver ATTRIBUTE_UNUSED,
...
#ifdef MTK_ART_COMMON
  if (ShouldFallback(compiler_driver, invoke_type, class_def_idx, method_idx, dex_file)) {
    return nullptr;
  }
#endif
