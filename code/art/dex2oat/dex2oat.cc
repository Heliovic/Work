...
// COE_SAFE_FUNCTION:Make dex2oat capable of recompiling .oat/.odex files
#ifdef __COE_SAFE_FUNCTION__
...
#ifdef MTK_SUSPEND_ALL_ACTIVE_THREADS_BEFORE_EXIT
 ...
 public:
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      // COE_SAFE_FUNCTION:Ignore unknown arguments and option values
      #ifndef __COE_SAFE_FUNCTION__
...
#if defined(MTK_ART_COMMON) && defined(MTK_ART_COMPILER_BACKEND)
      #if (MTK_ART_COMPILER_BACKEND == 0)
      compiler_kind_ = image_ ? Compiler::kQuick : Compiler::kQuick;
      #else
      compiler_kind_ = image_ ? Compiler::kOptimizing : Compiler::kOptimizing;
      #endif
#else
...
#if defined(MTK_ART_COMMON) && defined(HAVE_ANDROID_OS)
    char dex2oat_compiler_backend[PROPERTY_VALUE_MAX];
    if (property_get("dex2oat-compiler-backend", dex2oat_compiler_backend, NULL) > 0) {
      if (strcmp(dex2oat_compiler_backend, "quick") == 0) {
        compiler_kind_ = Compiler::kQuick;
      } else if (strcmp(dex2oat_compiler_backend, "optimizing") == 0) {
        compiler_kind_ = Compiler::kOptimizing;
      }
    }
#endif
...
// COE_SAFE_FUNCTION:Store Xposed-specific information in the oat file header
#ifdef __COE_SAFE_FUNCTION__
...
// COE_SAFE_FUNCTION:Store Xposed-specific information in the oat file header
  ...
  void Compile() {
    ...
    #ifdef MTK_ART_COMMON
    driver_ = GetCompilerDriver(compiler_options_.get(),
    #else
