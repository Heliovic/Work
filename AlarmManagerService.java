...
//Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
import android.os.Build;
import android.os.AsusEvtLogInternal;
//End:hungjie_tseng@asus.com, Adding for AsusEvtLog
...
class AlarmManagerService extends SystemService {
    ...
    public final static boolean ENABLE_POWER_CUSTOMIZE = true;
    public static final String ACTION_QUERY_WAKEUP_TIMES = "com.asus.action.query_wakeup_times";
    public static final String ACTION_ALARM_LIST_PREPARED = "android.asus.action.alarm_list_prepared";
    public static final String ACTION_QUERY_ALARM_INFO_CATEGORY = "com.asus.category.query_alarm_info";
    PowerCustomizeReceiver mPowerCustomizeReceiver;
    //End: hungjie_tseng@asus.com
    //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
    public static final String ACTION_CLEAR_WAKEUPTIMES = "action_clear_wakeuptimes";
    private static final long MSG_QUERY_INFO_DELAY = 1000; //1s
    private static final long MSG_CLEAR_WAKEUP_DELAY = 10 * 1000;//10s
    //End:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
    //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
    private static final boolean DEBUG_WAKEUP_ALARM = localLOGV || true;
    private String mNextWakeupAppName = "";
    private boolean mSupportAsusEvtLog = Build.FEATURES.ENABLE_ASUS_EVTLOG;
    AsusEvtLogInternal mAsusEvtLogInternal;
    //End:hungjie_tseng@asus.com, Adding for AsusEvtLog
    /**
        ...
        boolean hasWakeups() {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if ((a.type & TYPE_NONWAKEUP_MASK) == 0) {
                    //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
                    mNextWakeupAppName = new String(a.statsTag);
                    //End:hungjie_tseng@asus.com, Adding for AsusEvtLog
                    return true;
                }
            }
    ...
    public void onStart() {
        ...
        //Begin:hungjie_tseng@asus.com
        if(ENABLE_POWER_CUSTOMIZE) {
            mPowerCustomizeReceiver = new PowerCustomizeReceiver();
        }
        //End: hungjie_tseng@asus.com
...
//liangfeng@wind-mobi.com 2016.11.22 start
//add this code lead to MTK SchedulePowerOnOff become invalid
// BEGIN: oliver_hu@asus.com
            // If this is always-on alarm, change to correct type correspondingly.
            /*
            if (type == AlarmManager.AON_RTC || type == AlarmManager.AON_RTC_WAKEUP) {
                type = type - AlarmManager.AON_OFFSET;
                flags |= AlarmManager.FLAG_AON_ALLOW_WHILE_IDLE_UNRESTRICTED;
            }*/
// END: oliver_hu@asus.com
//liangfeng@wind-mobi.com 2016.11.22 end
        ...
        while (mAlarmBatches.size() > 0) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if ((alarm.flags&AlarmManager.FLAG_ALLOW_WHILE_IDLE) != 0
                        && (alarm.flags&AlarmManager.FLAG_AON_ALLOW_WHILE_IDLE_UNRESTRICTED) == 0) {
    ...
    private class AlarmThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while (true)
            {
                ...
                //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
                long nextWakeupRTC = 0;
                long diffTime = 0;
                String wakeupApp = "";
                //End:hungjie_tseng@asus.com, Adding for AsusEvtLog
                if (result != TIME_CHANGED_MASK) {
                    //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
                    nextWakeupRTC = mNextWakeup + (nowRTC - nowELAPSED);
                    diffTime = nowRTC - nextWakeupRTC;
                    wakeupApp = new String(mNextWakeupAppName);
                    //End:hungjie_tseng@asus.com, Adding for AsusEvtLog
                    ...
                    //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
                    printWakeupInfo(result, nextWakeupRTC, diffTime, wakeupApp);
                    //End:hungjie_tseng@asus.com, Adding for AsusEvtLog
        ...
        //Being:hungjie_tseng@asus.com, Adding for AsusEvtLog
        /*
         *  Adding this log to find who wakeup device
         *  , this is scenario for device sleeping
         *  , writing log to /proc/asusevtlog when triggered by android alarm
         */
        private void printWakeupInfo(int result, long nextWakeupRTC, long diffTime, String wakeupAppName) {
            boolean isWakeupType = ((result & IS_WAKEUP_MASK) != 0);
            if (isWakeupType && (diffTime >= 0) && !mInteractive) {
                if(DEBUG_WAKEUP_ALARM) {
                    StringBuilder asusLog = new StringBuilder(
                            "[Alarm][userspace]: Android alarm wakeup device when screen off")
                            .append(" nowRTC-nextWakeup diffTime: ").append(diffTime)
                            .append(" WakeupAppName: ").append(wakeupAppName);
                    Slog.d(TAG, asusLog.toString());
                }
                if(mSupportAsusEvtLog) {
                    StringBuilder asusEvtLog = new StringBuilder("[Alarm][userspace]:WakeupAppName: ")
                            .append(wakeupAppName);
                    if (mAsusEvtLogInternal == null) {
                        mAsusEvtLogInternal = LocalServices.getService(AsusEvtLogInternal.class);
                    }
                    if (mAsusEvtLogInternal != null) {
                        mAsusEvtLogInternal.writeAsusEvtLog(asusEvtLog.toString());
                    } else {
                        if (DEBUG_WAKEUP_ALARM) {
                            Slog.d(TAG, "Can't get AsusEvtLogService when printWakeupInfo");
                        }
                    }
                }
                return;
            ...
            if (!mInteractive) {
                if(DEBUG_WAKEUP_ALARM) {
                    StringBuilder asusLog = new StringBuilder(
                            "[Alarm][userspace]:Doesn't Android setting RTC time wakeup device")
                            .append(" nowRTC-nextWakeup diffTime: ").append(diffTime)
                            .append(" when screen off");
                    Slog.d(TAG, asusLog.toString());
                }
            }
        }
    ...
    private class AlarmHandler extends Handler {
        ...
        //Begin:hungjie_tseng@asus.com
        public static final int SEND_WAKEUP_TIMES_INFO = 100;
        //End:hungjie_tseng@asus.com
        //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
        public static final int CLEAR_WAKEUP_TIMES = 101;
        //End:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
        public AlarmHandler() {
        }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                //Begin:hungjie_tseng@asus.com
                case SEND_WAKEUP_TIMES_INFO:
                    getAlarmList();
                    break;
                //End:hungjie_tseng@asus.com

                //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
                case CLEAR_WAKEUP_TIMES:
                    synchronized (mLock) {
                        clearWakeupTimesLocked();
                    }
                    break;
                //End:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
    ...
    private void resetPoweroffAlarm(Alarm alarm) {
        ...
        if (mNativeData != 0 && mNativeData != -1) {
			...
			// hubo@wind-mobi.com add begin Feature#137636 2016/11/28
            else if (setPackageName.equals("com.asus.deskclock")) {
            ...
            // hubo@wind-mobi.com add end Feature#137636 2016/11/28		
    ...
    //Begin:hungjie_tseng@asus.com
    class PowerCustomizeReceiver extends BroadcastReceiver {
        public PowerCustomizeReceiver() {
            IntentFilter filter = new IntentFilter();
            filter.addAction(ACTION_QUERY_WAKEUP_TIMES);
            //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
            filter.addAction(ACTION_CLEAR_WAKEUPTIMES);
            //End:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
            filter.setPriority(IntentFilter.SYSTEM_HIGH_PRIORITY);
            filter.addCategory(ACTION_QUERY_ALARM_INFO_CATEGORY);
            getContext().registerReceiver(this, filter);
        }
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (ACTION_QUERY_WAKEUP_TIMES.equals(action)) {
                Slog.d(TAG, "PowerCustomizeReceiver onReceive query_wakeup_times");
                mHandler.removeMessages(AlarmHandler.SEND_WAKEUP_TIMES_INFO);
                mHandler.sendEmptyMessageDelayed(AlarmHandler.SEND_WAKEUP_TIMES_INFO, MSG_QUERY_INFO_DELAY);
            }
            //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
            if(ACTION_CLEAR_WAKEUPTIMES.equals(action)) {
                mHandler.removeMessages(AlarmHandler.CLEAR_WAKEUP_TIMES);
                mHandler.sendEmptyMessageDelayed(AlarmHandler.CLEAR_WAKEUP_TIMES, MSG_CLEAR_WAKEUP_DELAY);
            }
            //End:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
        }
    }
    ...
    public void getAlarmList() {
        Intent intent = new Intent(ACTION_ALARM_LIST_PREPARED);
        ...
        synchronized (mLock) {
            ...
            final int N = mAlarmBatches.size();
            for(int i = 0; i < N; i++) {
                size += mAlarmBatches.get(i).size();
            }

            arrayTarget = new String[size];
            arrayWakeupNum = new int[size];
            int index = 0;
            for (Batch batch : mAlarmBatches) {
                final int n = batch.size();
                for (int i = 0; i < n; i++) {
                    Alarm alarm = batch.get(i);
                    String pkgName = alarm.operation.getTargetPackage();
                    BroadcastStats bs = getStatsLocked(alarm.operation);

                    arrayTarget[index] = new String(pkgName);
                    arrayWakeupNum[index] = bs.numWakeup;
                    index++;
                }
            }

            intent.putExtra("alarm_list_pkg", arrayTarget);
            intent.putExtra("alarm_list_wakeupNum", arrayWakeupNum);
        }
        intent.setFlags(Intent.FLAG_RECEIVER_FOREGROUND | Intent.FLAG_RECEIVER_REGISTERED_ONLY);
        intent.addCategory(ACTION_QUERY_ALARM_INFO_CATEGORY);
        Slog.d(TAG, "PowerCustomizeReceiver getAlarmList sendBroadcast alarm_list_prepared");
        getContext().sendBroadcast(intent);
    }
    //End:hungjie_tseng@asus.com
    //Begin:hungjie_tseng@asus.com, Adding for clear alarm wakeup times
    private void clearWakeupTimesLocked() {
        Slog.d(TAG, "[Frameworks][AlarmManager][clearWakeupTimesLocked]perform clear wakeup");
        for (int iu = 0; iu < mBroadcastStats.size(); iu++) {
            ArrayMap<String, BroadcastStats> uidStats = mBroadcastStats.valueAt(iu);
            for (int ip = 0; ip < uidStats.size(); ip++) {
                BroadcastStats bs = uidStats.valueAt(ip);
